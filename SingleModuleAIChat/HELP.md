...
if we have subscription then use openAI
else use ollama

1. Open Ai is cloud base: add subscription in application.properties file(spring.ai.openai.api-key) and change the spring-ai-starter-model-openai
2. But Ollama is free so download and run ollama in local then update the prop file as per now